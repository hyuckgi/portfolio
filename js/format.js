...
fom = {
	...
	scene : function( args ) {
		...
		// app 영역에서 활성된 탭에 따른 scene 이동 제어
		...
		if ( fom.isSupport('csstransitions') && ! fom.isMobile() ) {
			...
			target.one('webkitTransitionEnd.fom msTransitionEnd.fom transitionend.fom', function() {
				$(this).unbind('webkitTransitionEnd.fom msTransitionEnd.fom transitionend.fom'); // 이벤트 중복 방지
				afterScene();
			});
		} else {
			beforeScene();
	...
	init : function() {
		...
		window.setTimeout(function() {
			$('#index.before .scroll').addClass('hide');
		}, 4000)

		// index roll
		$('#index .screenshot').addClass('active1').on('webkitAnimationIteration.fom animationiteration.fom', function() {
			var _this = $(this),
				next = $(this).is('.active1') ? 'active2' : $(this).is('.active2') ? 'active3' : $(this).is('.active3') ? 'active4' : 'active1';

			$(this).removeClass('active1 active2 active3 active4');
			$(this).addClass( next );
		});

		// random visual
		$('#index .visual').addClass( 'visual' + ( Math.floor(Math.random() * 3) + 1 ) );
		...
		// object scrollspy
		var objSpy = $('#index .title, #index .day, #index .visual, #index .obj1, #index .obj2, #index .phone, #index .scroll, #index .download, #ev .title, #ev .txt, #ev .bt, #app .txt, #app #scene-sub2, #faq h2, #faq .tab, #faq .list, #skill');
